...
static void can_open_schema_data01() {
  ...
  assert_eq_str(schema.children.elements[0][0].name, "date", "should have a name for the first child");
  assert(schema.children.elements[0][0].children.count == 0, "should have no children");
  assert(schema.children.elements[0][0].children.elements == NULL, "should have no children elements");

  assert(schema.children.elements[0][0].repeated_type == PARQUET_REPETITION_TYPE_OPTIONAL, "should be optional");
  assert(schema.children.elements[0][0].converted_type == PARQUET_CONVERTED_TYPE_DATE, "should be date");
  assert(schema.children.elements[0][0].data_type == PARQUET_DATA_TYPE_INT32, "should have int32 data type");
  assert(schema.children.elements[0][0].type_length == PARQUET_UNKNOWN_VALUE, "should have unknown type length");
